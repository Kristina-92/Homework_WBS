...
const login = async (req, res) => {
  try {
    ...
    if (!bcryptjs.compareSync(password, account.password)) {
      await updateLoginStatus(account._id, "fail");
      return res
        .status(400)
        .send({ message: "Log in failed! Wrong password!" });
    }
    ...
    await updateLoginStatus(account._id, "success");
    return res.status(200).send({ message: "Log in successful", token });
  } catch (err) {
    console.log(err);
